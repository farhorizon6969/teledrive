...
func (d *DB) CreateSnapshot() (string, error) {
	...
	rawSnapshotPath := filepath.Join(tempDir, fmt.Sprintf("teledrive-raw-%s.db", timestamp))
	...
	gzPath := filepath.Join(tempDir, fmt.Sprintf("teledrive-backup-%s.db.gz", timestamp))
