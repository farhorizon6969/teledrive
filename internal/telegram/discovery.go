...
// DiscoverStorageChannels queries Telegram dialogs for existing "TeleDrive Vault" broadcast channels.
func (m *ClientManager) DiscoverStorageChannels(ctx context.Context) ([]DiscoveredChannel, error) {
	...
	for _, chat := range chats {
		if c, ok := chat.(*tg.Channel); ok {
			if c.Broadcast && c.Title == "TeleDrive Vault" {
...
// CreateStorageChannel creates a new private Telegram Storage Channel "TeleDrive Vault".
...
func (m *ClientManager) OnboardStorageChannelInteractive(ctx context.Context, reader *bufio.Reader, destDBPath string) error {
	...
	if len(candidates) == 0 {
		...
		fmt.Printf("Found existing Storage Channel 'TeleDrive Vault' (ID: %d, Created: %s, %s)\n",
			cand.ID, cand.CreatedAt.Format("2006-01-02"), snapText)
		...
		fmt.Printf("\nFound %d Storage Channels named 'TeleDrive Vault':\n", len(candidates))
		for i, cand := range candidates {
			...
			fmt.Printf("  [%d] Channel ID: %d | Created: %s | %s\n",
				i+1, cand.ID, cand.CreatedAt.Format("2006-01-02"), snapText)
		}
	...
	if selected.SnapshotCount > 0 && selected.LatestSnapshot != nil {
		...
		if promptYesNo(reader, "Restore database from this snapshot now? [Y/n]: ", true) {
			...
			fmt.Println("Skipping restore. TeleDrive will run with current local database.")
		}
	} else {
		fmt.Println("No backup snapshots found in this channel. TeleDrive will run with a fresh database.")
	}
